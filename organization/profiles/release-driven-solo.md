...
ProtoType の current release-driven development で使用する Operating Model を定義します。
...
- pnpm / package.json / lockfile が application dependency/tooling authority。Bun は Agent Skills CLI 用の auxiliary tool としてのみ導入する。
- Vite/TypeScript/React build/test entry points は existing package scripts を authority とする。
- browser/UI validation と source-only typecheck/lint/build evidence を分離する。
